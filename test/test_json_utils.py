...
from src.transform_lambda.json_utils import write_json_data
...
class TestWritejson:
    def test_function_writes_to_s3_bucket(self, s3_client):
        ...
        df = pd.DataFrame(d)
        write_json_data(df, f"s3://{bucket}/movie.json", session)
        result = s3_client.list_objects_v2(Bucket=bucket)
        assert result["Contents"][0]["Key"] == f"movie.json"
    ...
    def test_function_writes_correct_data_to_s3_bucket(self, s3_client):
        ...
        df = pd.DataFrame(d)
        write_json_data(df, f"s3://{bucket}/movie.json", session)
        result = wr.s3.read_json(path=f"s3://{bucket}/movie.json")
    ...
    def test_missing_bucket_raises_client_error(self, s3_client):
        ...
        d = {"F Name": ["John", "Steve", "Stefani"]}
        df = pd.DataFrame(d)
        result = write_json_data(df, f"s3://fake_bucket/movie.json", session)
